...
<div {{ $attributes->class(['fieldset'])}}>
  ...
  <input type="{{$type}}" id="{{$id}}" name="{{$name}}" value="{{ old($name, $value) }}" placeholder="{{$placeholder}}" @required($required) @disabled($disabled) class="block {{$inputW}} shadow-sm p-3 xl:py-1 rounded-md border border-black/5 bg-white text-gray-600 disabled:bg-transparent disabled:text-inherit" />
