<x-admin.layout>
  <section class="grid grid-cols-2 mb-5">
    <div>
      <h1 class="">Pago #{{$Payment->id}}</h1>
      <h3 class="">Para: {{$Payment->user->name}}</h3>
      <p>Fecha: {{ $Payment->created_at->locale('es')->isoFormat('d MMM Y') }}</p>
  ...
  <section class="my-5 shadow-md rounded-full px-5 py-3 flex flex-wrap justify-around bg-white dark:bg-white/10 text-center ">
    ...
    <div class="">
      <h4>Total</h4>
      <p class="text-3xl font-serif">$@money($Payment->amount)</p>
    </div>
  </section>
